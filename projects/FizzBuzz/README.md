# Algorand Python Project - FizzBuzz in the AVM

This example uses an Algorand smart contract written in Algorand Python (Puya) to demonstrate using the capabilities of the Algorand virtual machine (AVM) to solve the well-known programming problem and interview question known as "FizzBuzz."

The smart contract features a few notable techniques:

- Looping over a range to carry out the solution
- Control flow in the core FizzBuzz algorithm
- Creating a dynamic array and appending value to it
- Converting integers to ASCII bytes strings
- Obtaining additional opcode budget for the program
...
By default the template creates a single `HelloWorld` contract under fizzbuzz folder in the `smart_contracts` directory. To add a new contract:
